...
philosophy, in one of five distinct visual voices, with a router that picks
...
skills/dsgn/
  ...
  agents/                     # 5 self-contained style personas
...
The reference docs are a snapshot. If you add new components to
`packages/registry/`, add new philosophy pillars, or change the token system
in `apps/site/app/globals.css`, update the matching file under
`skills/dsgn/reference/` — there's no automated sync.
