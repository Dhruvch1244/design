...
export interface ShowcaseSite {
  ...
  // first commit message, with one deliberate exception: the literal local
  // filesystem path each prompt was dispatched from (a real Windows
  // username + directory) is redacted to `~/workspace/...`. That's a
  // machine-local implementation detail, not part of the actual brief —
  // redacting it doesn't change what was asked for. Never paraphrase
...
Where to build it: a brand-new standalone Next.js project in a fresh sibling directory, ~/workspace/dsgn-showcase-analytics — NOT inside the ~/workspace/Design monorepo. This needs to be a genuine external consumer of the published package (npx @dhruvchoudhary/dsgn add <component>), not an internal workspace shortcut — that's the actual point of building it as a separate project: it's the first real end-to-end test of someone installing this tool fresh.
...
Where to build it: a brand-new standalone Next.js project in a fresh sibling directory, ~/workspace/dsgn-showcase-tasks — NOT inside the ~/workspace/Design monorepo. This must be a genuine external consumer of the published package (npx @dhruvchoudhary/dsgn add <component>), not a hand-copied shortcut.
...
Where to build it: a brand-new standalone Next.js project in a fresh sibling directory, ~/workspace/dsgn-showcase-social — NOT inside the ~/workspace/Design monorepo. This must be a genuine external consumer of the published package (npx @dhruvchoudhary/dsgn add <component>), not a hand-copied shortcut.
...
**Where to build it**: a brand-new standalone Next.js project in a fresh sibling directory, \`~/workspace/dsgn-showcase-shop\` — NOT inside the \`~/workspace/Design\` monorepo. This must be a genuine external consumer of the published package (\`npx @dhruvchoudhary/dsgn add <component>\`), not a hand-copied shortcut.
